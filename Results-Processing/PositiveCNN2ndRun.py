...
for i in range(rows):
    ...
    accuracy_values = pd.read_csv(
        "Results/PositiveCNN2ndRun.csv", skiprows=(i) * 6, nrows=6, header=None
    )
    accuracy_values.columns = [0, 1]
    cnn_mae = 0
    cnn_ = 0
    for j in range(6):
        cnn_mae += accuracy_values.loc[j].iat[0]
        cnn_ += accuracy_values.loc[j].iat[1]
    col.append(cnn_mae / 6)
    col.append(cnn_ / 6)
    arr.append(col)

df = pd.DataFrame(arr, columns=["Attribute", "CNN MAE", "CNN F1"])
...
df_diff = pd.DataFrame(arr2, columns=["Attribute", "CNN MAE", "CNN F1"])
