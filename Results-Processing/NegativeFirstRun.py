...
for i in range(rows):
    ...
    accuracy_values = pd.read_csv(
        "Results/1stRun.csv", skiprows=(i) * 12, nrows=12, header=None
    )
...
df = pd.DataFrame(arr, columns=["Attribute", "LSTM MAE", "CNN MAE", "LSTM  ", "CNN  "])
print(df.sort_values(by=["CNN MAE"]))
...
df_diff = pd.DataFrame(
    arr2, columns=["Attribute", "LSTM MAE", "CNN MAE", "LSTM  ", "CNN  "]
)
...
for i in range(1, 22):
    if df_diff.loc[i].iat[1] * df_diff.loc[i].iat[2] > 0:
        if df_diff.loc[i].iat[1] > 0:
            same_list.append(
                df_diff.loc[i].iat[0] + " improved the model accuracy"
            )  # how to get this to work, it still rearranges the index
        else:
            same_list.append(df_diff.loc[i].iat[0] + " decreased the model accuracy")
    else:
